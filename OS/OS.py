...
class EazyOSDesktop:
    ...
    def __init__(self):
        ...
        self.root.after(100, self._play_startup_sound_and_msg)
        
    def msg(self):
        self._show_messagebox(
            "warning",
            "EazyOS Pre-Release Message",
            "You are running a pre-release version of EazyOS.\nThis version is still in development, there may be unknown bugs.\nIf you believe this is an error, please contact us. "
        )
    ...
    def _get_config_mtime(self) -> float:
        try:
            ...
            return 0.0
    def _play_startup_sound_and_msg(self):
    ...
    def _create_start_menu(self) -> None:
        ...
        self.start_menu.add_command(label="EazyOS 3 Pre-Release 1", state="disabled")
        ...
        info_menu = tk.Menu(self.start_menu, tearoff=0, bg="white", fg="black",
                                   activebackground=WIN98_BLUE, activeforeground="white")
        info_menu.add_command(label="Contact Me", command=self._open_contact)
        info_menu.add_command(label="Release Notes", command=self._open_notes)
        info_menu.add_command(label="About EazyOS", command=self._open_about)
        self.start_menu.add_cascade(label="Extra Information", menu=info_menu)
    ...
    def _create_context_menu(self) -> None:
        self.context_menu = tk.Menu(self.root, tearoff=0, bg="white", fg="black",
                                    activebackground=WIN98_BLUE, activeforeground="white")
        self.context_menu.add_command(label="EazyOS 3 Pre-Release 1", state="disabled")
    ...
    def _open_notes(self):
        # os.path.join handles all the tricky slashes for you
        script_path = os.path.join(PROGRAM_FILES_DIR, "notepad", "notepad.py")
        target_file = os.path.join(SYSTEM_DIR, "notes.txt")
 
        subprocess.run(["py", script_path, target_file])
    ...
    def _restart(self):
        SoundManager.play('DING.wav')
        if self._show_messagebox("askyesno", "Restart?", "Are you sure you want to restart?"):
    ...
    def _logout(self):
        SoundManager.play('DING.wav')
        if self._show_messagebox("askyesno", "Logout?", "Are you sure you want to log out?"):
            self.taskbar_manager.show()
            self.root.destroy()
    ...
    def _exit(self):
        SoundManager.play('DING.wav')
        if self._show_messagebox("askyesno", "Shutdown?", "Are you sure you want to Shutdown?"):
            SoundManager.play('shutdown.wav')
            time.sleep(4)
            self.taskbar_manager.show()
            self.root.destroy()
    ...
    def run(self):          # <-- indented to class level
        try:
