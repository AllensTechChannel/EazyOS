...
class ConfigManager:
    ...
    def get_system(config: dict) -> dict:
        """Convenience accessor for the system section."""
    ...
    def bool_val(config: dict, section: str, key: str, default: bool = True) -> bool:
        """Read a string 'true'/'false' value as a Python bool."""
...
class ThemeManager:
    ...
    def apply_from_config(self) -> None:
        """Re-read config file and apply ALL theme + system settings live."""
        self.config = ConfigManager.load_config()
        ...
        # Window title bar color on any open Toplevels
...
class EazyOSDesktop:
    ...
    def __init__(self):
        ...
        # Apply system settings from config (sounds, clock, etc.)
        # Apply system settings first so ENABLE_SOUNDS is set before the startup sound
        self._apply_system_settings()

        # Poll for config changes made by regedit every 2 seconds
    ...
    def _poll_config(self) -> None:
        """Check every 2s if regedit has written a new config, and reload if so."""
        try:
    ...
    def _apply_system_settings(self) -> None:
        """Apply system section of config to the running desktop."""
    ...
    def _create_desktop_icons(self) -> None:
        ...
        icon_configs = [
            ...
            ("display_properties.png", self._open_personalization, "Display"),
    ...
    def _create_start_menu(self) -> None:
        ...
        # Programs submenu (contains System Programs nested inside)
        programs_menu = self._create_programs_menu()
        self.start_menu.add_cascade(label="Programs", menu=programs_menu)

        self.start_menu.add_separator()

        personal_menu = tk.Menu(self.start_menu, tearoff=0, bg="white", fg="black",
                                activebackground=WIN98_BLUE, activeforeground="white")
        personal_menu.add_command(label="Wallpapers, Colors, Settings",
                                  command=self._open_personalization)
        self.start_menu.add_cascade(label="Personalization", menu=personal_menu)

        self.start_menu.add_command(label="Add Username and Password", command=self._manage_users)
        ...
        self.start_menu.add_command(label="Exit", command=self._exit)

        self.start_button.config(command=self._show_start_menu)

    def _create_sys_program(self) -> tk.Menu:
        """Create the System Programs submenu"""
    ...
    def _create_programs_menu(self) -> tk.Menu:
        ...
        # Nest System Programs inside Programs
    ...
    def _create_context_menu(self) -> None:
        ...
        self.context_menu.add_command(label="Personalization", command=self._open_personalization)
    ...
    def _open_dos_prompt(self):
        script = os.path.join(SYSTEM_DIR, "CMD.py")
    ...
    def _open_update(self):
        ...
        if self._show_messagebox("askyesno", "Update", "Are you sure you want to upgrade?"):
            script = os.path.join(BASE_DIR, "setup", "UPDATEOS.py")
    ...
    def _manage_users(self):
        bat_path = os.path.join(SYSTEM_DIR, "User.bat")
        if not os.path.exists(bat_path):
            self._show_messagebox("error", "Error", "User.bat not found!")
            return
        self._run_bat_terminal(bat_path)

    def _run_bat_terminal(self, bat_path: str):
        win = self.window_manager.create_window("Command Prompt", width=700, height=500)

        for widget in win.winfo_children():
            if isinstance(widget, tk.Frame) and widget.cget('bg') == "#FAF9F6":
                body = widget
                break
        else:
            return
        ...
        text_widget = tk.Text(body, bg="black", fg="white",
    ...
    def _open_personalization(self):
        wallpaper_menu = tk.Menu(self.root, tearoff=0, bg="white", fg="black",
                                 activebackground=WIN98_BLUE, activeforeground="white")

        def pick_color():
            color = colorchooser.askcolor(title="Choose Background Color")
            if color and color[1]:
                self.theme_manager.apply_theme(color=color[1])
        ...
        for file in os.listdir(WALLPAPER_DIR):
            if file.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp')):
                ...
                wallpaper_submenu.add_command(
                    label=file,
                    command=lambda p=path: self.theme_manager.apply_theme(image_path=p)
                )

        if wallpaper_submenu.index("end") is not None:
        ...
        def select_custom():
            ...
            if file_path:
                self.theme_manager.apply_theme(image_path=file_path)

        wallpaper_menu.add_command(label="Select From File...", command=select_custom)
        wallpaper_menu.add_separator()

        def reset():
            self.theme_manager.reset_to_default()
            self._show_messagebox("info", "Reset", "Theme reset to Windows 98 default.")

        wallpaper_menu.add_command(label="Reset to Default", command=reset)

        try:
            wallpaper_menu.tk_popup(self.root.winfo_pointerx(), self.root.winfo_pointery())
        finally:
    ...
    def _logout(self):
        ...
        if self._show_messagebox("askokcancel", "Logout", "Are you sure you want to log out?"):
            self.taskbar_manager.show()
            self.root.destroy()
            SoundManager.play('win98logoff.wav')
            subprocess.run([sys.executable, os.path.join(BASE_DIR, "login.py")])

    def _exit(self):
        SoundManager.play('DING.wav')
        if self._show_messagebox("askokcancel", "Exit", "Are you sure you want to exit?"):
            SoundManager.play('CHIMES.wav')
            self.taskbar_manager.show()
            self.root.destroy()
